...
// ----------------------------------------------------------------------------
// Helper: Convert TimeSourceTier to string
// ----------------------------------------------------------------------------
inline const char* tier_to_string(TimeSourceTier tier) {
    switch (tier) {
        case TimeSourceTier::TIER1_GPSDO:    return "GPSDO (Tier 1)";
        case TimeSourceTier::TIER2_EXTERNAL: return "External PPS (Tier 2)";
        case TimeSourceTier::TIER3_INTERNAL: return "Internal (Tier 3)";
        default:                              return "Unknown";
    }
}

// ----------------------------------------------------------------------------
// NEW FUNCTION: Query GPS time from GPSDO sensor
// ----------------------------------------------------------------------------
bool query_gps_time_from_sensor(
    uhd::rfnoc::mb_controller::sptr mb_controller,
    std::time_t& gps_time)
{
    try {
        auto sensor_names = mb_controller->get_sensor_names();
        
        // Check for GPS time sensor
        if (std::find(sensor_names.begin(), sensor_names.end(), "gps_time") 
            == sensor_names.end()) {
            return false;
        }
        
        // GPS time sensor returns Unix time on most GPSDO modules
        auto gps_sensor = mb_controller->get_sensor("gps_time");
        gps_time = static_cast<std::time_t>(gps_sensor.to_int());
        
        return true;
    } catch (const std::exception& e) {
        std::cerr << "  GPS time query failed: " << e.what() << std::endl;
        return false;
    }
}

// ----------------------------------------------------------------------------
// NEW FUNCTION: Check GPS lock status
// ----------------------------------------------------------------------------
bool check_gps_locked(uhd::rfnoc::mb_controller::sptr mb_controller)
{
    try {
        auto sensor_names = mb_controller->get_sensor_names();
...
// ----------------------------------------------------------------------------
// NEW FUNCTION: Check reference lock status
// ----------------------------------------------------------------------------
bool check_ref_locked(uhd::rfnoc::mb_controller::sptr mb_controller)
{
    try {
        auto sensor_names = mb_controller->get_sensor_names();
...
// ----------------------------------------------------------------------------
// NEW FUNCTION: Detect and configure best time source
// ----------------------------------------------------------------------------
TimeSourceTier detect_and_configure_time_source(
    uhd::rfnoc::mb_controller::sptr mb_controller,
    const PpsResetConfig& config,
    std::string& actual_time_source,
    std::string& actual_clock_source)
{
    std::cout << "\n[Time Source Detection]" << std::endl;
    
    auto time_sources = mb_controller->get_time_sources();
    auto clock_sources = mb_controller->get_clock_sources();
    
    std::cout << "  Available time sources: ";
    for (const auto& s : time_sources) std::cout << s << " ";
    std::cout << std::endl;
    
    std::cout << "  Available clock sources: ";
    for (const auto& s : clock_sources) std::cout << s << " ";
    std::cout << std::endl;
    
    auto has_source = [](const std::vector<std::string>& sources, const std::string& target) {
    ...
    // TRY TIER 1: GPSDO
    // =========================================================================
    if (has_source(time_sources, "gpsdo") && has_source(clock_sources, "gpsdo")) {
        std::cout << "  Attempting GPSDO configuration..." << std::endl;
        ...
        try {
            ...
            const double gps_timeout = 30.0;  // 30 second timeout
            
            while (true) {
                if (check_gps_locked(mb_controller)) {
                    std::cout << " LOCKED" << std::endl;
                    actual_time_source = "gpsdo";
                    actual_clock_source = "gpsdo";
                    return TimeSourceTier::TIER1_GPSDO;
                }
            ...
            std::cout << "  GPS lock failed, falling back..." << std::endl;
        } catch (const std::exception& e) {
            std::cout << "  GPSDO config failed: " << e.what() << std::endl;
        }
    }
    
    // =========================================================================
    // TRY TIER 2: External PPS
    // =========================================================================
    if (has_source(time_sources, "external")) {
        std::cout << "  Attempting external PPS configuration..." << std::endl;
        
        try {
            if (has_source(clock_sources, "external")) {
                mb_controller->set_clock_source("external");
                actual_clock_source = "external";
            } else {
                mb_controller->set_clock_source("internal");
                actual_clock_source = "internal";
            }
            ...
            while (true) {
                if (check_ref_locked(mb_controller)) {
                    std::cout << " LOCKED" << std::endl;
                    break;
                }
            ...
            actual_time_source = "external";
            return TimeSourceTier::TIER2_EXTERNAL;
            
        } catch (const std::exception& e) {
            std::cout << "  External PPS config failed: " << e.what() << std::endl;
        }
    }
    
    // =========================================================================
    // FALLBACK TIER 3: Internal
    // =========================================================================
    std::cout << "  Using internal clock/PPS (Tier 3)" << std::endl;
    ...
    actual_time_source = "internal";
    actual_clock_source = "internal";
    
    return TimeSourceTier::TIER3_INTERNAL;
}

// ----------------------------------------------------------------------------
// NEW FUNCTION: establish_time_anchor - REPLACES perform_pps_reset for TSI use
// 
// This function establishes the correlation between hardware time and 
// wall-clock/GPS time at a PPS edge. The returned TimeAnchor must be used
// for ALL subsequent TSI packet header generation.
// ----------------------------------------------------------------------------
TimeAnchor establish_time_anchor(
    ...
    const PpsResetConfig& config)
{
    ...
    if (!config.enable_pps_reset) {
        std::cout << "PPS reset disabled - using wall clock fallback" << std::endl;
        anchor.unix_time_at_anchor = std::chrono::system_clock::to_time_t(
            std::chrono::system_clock::now());
        anchor.hw_secs_at_anchor = 0;
        anchor.tier = TimeSourceTier::TIER3_INTERNAL;
        anchor.valid = true;  // Valid but low accuracy
        return anchor;
    }
    
    std::cout << "\n========================================" << std::endl;
    std::cout << "  ESTABLISHING TIME ANCHOR FOR TSI" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    try {
        ...
        anchor.tick_rate = timekeeper->get_tick_rate();
        std::cout << "  Tick rate: " << (anchor.tick_rate / 1e6) << " MHz" << std::endl;
        
        // Detect and configure best available time source
        anchor.tier = detect_and_configure_time_source(
            mb_controller, config, 
            anchor.time_source, anchor.clock_source);
        
        std::cout << "\n  Operating in: " << tier_to_string(anchor.tier) << std::endl;
        
        // Small delay for source switch to settle
        std::this_thread::sleep_for(std::chrono::milliseconds(300));
        
        // =====================================================================
        // TIER-SPECIFIC ANCHOR ESTABLISHMENT
        // =====================================================================
        
        if (anchor.tier == TimeSourceTier::TIER1_GPSDO) {
            // -----------------------------------------------------------------
            // TIER 1: Use GPS time directly
            // -----------------------------------------------------------------
            std::cout << "\n[Tier 1: GPS Time Alignment]" << std::endl;
            
            anchor.gps_locked = check_gps_locked(mb_controller);
            std::cout << "  GPS locked: " << (anchor.gps_locked ? "YES" : "NO") << std::endl;
            
            // Query GPS time
            std::time_t gps_time;
            if (query_gps_time_from_sensor(mb_controller, gps_time)) {
                std::cout << "  GPS time: " << gps_time << " (Unix epoch)" << std::endl;
                
                // GPS time corresponds to LAST PPS, so NEXT PPS is gps_time + 1
                uhd::time_spec_t time_to_set(static_cast<double>(gps_time + 1));
                timekeeper->set_time_next_pps(time_to_set);
                
                anchor.unix_time_at_anchor = gps_time + 1;
                anchor.hw_secs_at_anchor = gps_time + 1;
                
                std::cout << "  Setting hardware time to " << (gps_time + 1) 
                          << " at next PPS" << std::endl;
            } else {
                // No GPS time sensor - fall back to system time but keep GPSDO PPS
                std::cout << "  GPS time sensor not available, using system time" << std::endl;
                anchor.tier = TimeSourceTier::TIER2_EXTERNAL;
                // goto tier2_anchor;  // Use Tier 2 logic for anchor establishment
            }
...
//         }
        ...
        if(anchor.tier != TimeSourceTier::TIER1_GPSDO || anchor.hw_secs_at_anchor == 0) {
            // -----------------------------------------------------------------
            // TIER 2 & 3: Detect PPS edge and capture system time
            // -----------------------------------------------------------------
            std::cout << "\n[Tier " << (anchor.tier == TimeSourceTier::TIER2_EXTERNAL ? "2" : "3")
                      << ": PPS Edge Detection]" << std::endl;
            
            // Get current last_pps time to detect edge
            uhd::time_spec_t last_pps = timekeeper->get_time_last_pps();
            std::cout << "  Last PPS time: " << last_pps.get_real_secs() << " s" << std::endl;
            
            // Wait for PPS edge
            std::cout << "  Waiting for PPS edge..." << std::flush;
            auto wait_start = std::chrono::steady_clock::now();
            bool edge_detected = false;
            
            while (true) {
                uhd::time_spec_t current_last_pps = timekeeper->get_time_last_pps();
                
                if (current_last_pps.get_real_secs() != last_pps.get_real_secs()) {
                    // PPS edge detected! Capture system time IMMEDIATELY
                    auto now = std::chrono::system_clock::now();
                    anchor.unix_time_at_anchor = std::chrono::system_clock::to_time_t(now);
                    edge_detected = true;
                    
                    std::cout << " detected!" << std::endl;
                    std::cout << "  System time at PPS: " << anchor.unix_time_at_anchor << std::endl;
                    break;
                }
                ...
                if (elapsed > 2.5) {
                    std::cout << "\n  WARNING: PPS edge not detected within timeout!" << std::endl;
                    
                    if (anchor.tier == TimeSourceTier::TIER3_INTERNAL) {
                        // For internal, proceed without PPS alignment
                        auto now = std::chrono::system_clock::now();
                        anchor.unix_time_at_anchor = std::chrono::system_clock::to_time_t(now);
                        edge_detected = true;
                        std::cout << "  Using current system time as anchor" << std::endl;
                    }
                    break;
                }
            ...
            if (!edge_detected) {
                std::cerr << "  ERROR: Failed to establish time anchor!" << std::endl;
                anchor.valid = false;
                return anchor;
            }
            
            // Set hardware time to 0 at next PPS
            std::cout << "  Setting hardware time to 0 at next PPS" << std::endl;
            timekeeper->set_ticks_next_pps(0);
            anchor.hw_secs_at_anchor = 0;
            anchor.unix_time_at_anchor += 1;  // The reset happens at NEXT PPS
        }
        
        // Wait for PPS reset to take effect
        std::cout << "  Waiting for PPS reset to complete..." << std::flush;
        ...
        if (config.verify_reset) {
            ...
            double time_secs = current_time.get_real_secs();
            
            std::cout << "  Current hardware time: " << std::fixed 
                      << std::setprecision(6) << time_secs << " s" << std::endl;
            
            if (anchor.tier == TimeSourceTier::TIER1_GPSDO && anchor.hw_secs_at_anchor > 0) {
                // GPS mode: verify we're close to expected UTC
                double expected = static_cast<double>(anchor.hw_secs_at_anchor);
                double delta = time_secs - expected;
                
                if (delta >= 0 && delta < 2.0) {
                    std::cout << "  GPS time alignment verified (" << (delta * 1000) 
                              << " ms since set)" << std::endl;
                } else {
                    std::cerr << "  WARNING: GPS time may not be aligned correctly!" << std::endl;
                }
            } else {
                // Non-GPS mode: verify time is small (reset to 0 worked)
                if (time_secs < 2.0) {
                    std::cout << "  Time reset verified (< 2 seconds)" << std::endl;
                } else {
                    std::cerr << "  WARNING: Time reset may have failed!" << std::endl;
                }
            }
        }
        ...
        std::cout << "\n========================================" << std::endl;
        std::cout << "  TIME ANCHOR ESTABLISHED" << std::endl;
        std::cout << "========================================" << std::endl;
        std::cout << "  Tier: " << tier_to_string(anchor.tier) << std::endl;
        std::cout << "  Unix time at anchor: " << anchor.unix_time_at_anchor << std::endl;
        std::cout << "  Hardware secs at anchor: " << anchor.hw_secs_at_anchor << std::endl;
        std::cout << "  Time source: " << anchor.time_source << std::endl;
        std::cout << "  Clock source: " << anchor.clock_source << std::endl;
        
        // Print the anchor time in human-readable format
        std::tm* tm_anchor = std::gmtime(&anchor.unix_time_at_anchor);
        std::cout << "  Anchor UTC: " 
                  << (tm_anchor->tm_year + 1900) << "-"
                  << std::setw(2) << std::setfill('0') << (tm_anchor->tm_mon + 1) << "-"
                  << std::setw(2) << std::setfill('0') << tm_anchor->tm_mday << " "
                  << std::setw(2) << std::setfill('0') << tm_anchor->tm_hour << ":"
                  << std::setw(2) << std::setfill('0') << tm_anchor->tm_min << ":"
                  << std::setw(2) << std::setfill('0') << tm_anchor->tm_sec << " UTC"
                  << std::endl;
        std::cout << "========================================\n" << std::endl;
        ...
        std::cerr << "Error establishing time anchor: " << e.what() << std::endl;
        anchor.valid = false;
        return anchor;
    }
}
...
// ----------------------------------------------------------------------------
// MODIFIED FUNCTION: timestamp_to_tsi_time
// Now properly uses the passed-in anchor (no changes needed if anchor is valid)
// ----------------------------------------------------------------------------
inline TsiTimeComponents timestamp_to_tsi_time_v2(
    ...
    const TimeAnchor& anchor)
{
    ...
    // Calculate absolute Unix time from hardware timestamp
    std::time_t abs_unix_time;
    
    if (anchor.tier == TimeSourceTier::TIER1_GPSDO && anchor.hw_secs_at_anchor > 0) {
        // Tier 1 with GPS time: hardware seconds ARE Unix seconds
        abs_unix_time = static_cast<std::time_t>(timestamp.get_full_secs());
    } else {
        // Tier 2/3: Calculate from anchor offset
        int64_t hw_delta_secs = timestamp.get_full_secs() - anchor.hw_secs_at_anchor;
        abs_unix_time = anchor.unix_time_at_anchor + hw_delta_secs;
    }
    ...
    // Fractional seconds in 5ns units (for TSI format)
    // At 200MHz: 1 tick = 5ns
    double frac_secs = timestamp.get_frac_secs();
    tc.frac_5ns = static_cast<uint32_t>(frac_secs * 200e6);
 ...
 */
inline packetheader build_tsi_header_from_packet_v2(
    const PacketBuffer& pkt,
    const TimeAnchor& anchor,      // <-- PASSED IN from tsi_config
    size_t stream_id,
    uint16_t sat_id,
    uint32_t tuning_freq_hz)
{
    ...
    // Time components - USE THE PASSED ANCHOR!
    TsiTimeComponents tc;
    if (pkt.has_timestamp && anchor.valid) {
        tc = timestamp_to_tsi_time_v2(pkt.timestamp, anchor);
    } else {
...
// ----------------------------------------------------------------------------
// MODIFIED FUNCTION: tsi_file_writer_thread
// 
// KEY CHANGE: Uses anchor from tsi_config instead of per-packet creation
// ----------------------------------------------------------------------------
// =============================================================================
 ...
 */
void tsi_file_writer_thread_v2(
    StreamContext& ctx,
    ...
    const TsiOutputConfig& tsi_config)
{
    ...
    // Log anchor info
    std::cout << "[TSI Writer " << ctx.stream_id << "] Using time anchor:" << std::endl;
    std::cout << "  Tier: " << tier_to_string(tsi_config.time_anchor.tier) << std::endl;
    std::cout << "  Valid: " << (tsi_config.time_anchor.valid ? "YES" : "NO") << std::endl;
    if (tsi_config.time_anchor.valid) {
        std::tm* tm_anchor = std::gmtime(&tsi_config.time_anchor.unix_time_at_anchor);
        std::cout << "  Anchor UTC: " 
                  << (tm_anchor->tm_year + 1900) << "-"
                  << std::setw(2) << std::setfill('0') << (tm_anchor->tm_mon + 1) << "-"
                  << std::setw(2) << std::setfill('0') << tm_anchor->tm_mday << " "
    ...
    if (tsi_config.csv_max_packets > 0) {
        ...
        std::cout << "[TSI Writer " << ctx.stream_id << "] CSV verification enabled"
                  << std::endl;
    }
        ...
        if (!write_batch.empty()) {
            try {
                for (const auto& pkt : write_batch) {
                    // Build TSI header using the ANCHOR FROM CONFIG
                    packetheader header = build_tsi_header_from_packet_v2(
                        pkt,
                        tsi_config.time_anchor,   // <-- USE THE ANCHOR!
                        ctx.stream_id,
                        tsi_config.sat_id,
                ...
                for (const auto& [prop, value] : props) {
                    ...
                    try {
                        if (prop_name == "coefficients" || prop_name == "coeffs") {
                            std::cout << "  Setting FIR coefficients for channel " << chan << std::endl;
                            // std::cout << "  Coefficient string: " << value << std::endl;

                            auto coeff_string = properties.at(block_id_str).at((prop_name + "/" + std::to_string(chan)));
                            // Parse coefficient string: comma or space separated
                            std::vector<int16_t> coeffs;
                            // std::stringstream ss(value);
                            std::stringstream ss(coeff_string);
                            std::cout << "  Coefficient string: " << coeff_string << std::endl;
                            std::string token;
                            char delim = (coeff_string.find(',') != std::string::npos) ? ','
                                                                                       : ' ';
    ...
    const TsiOutputConfig& tsi_config)
{
    ...
    if (tsi_config.enabled) {
        std::cout << "\nEstablishing time anchor for TSI output..." << std::endl;
        actual_tsi_config.time_anchor = establish_time_anchor(graph, config.pps_reset);
        
        if (!actual_tsi_config.time_anchor.valid) {
            std::cerr << "WARNING: Time anchor not valid, TSI timestamps may be inaccurate!"
                      << std::endl;
        }
    }
    
    // Create TSI writer threads vector
    std::vector<std::unique_ptr<std::thread>> writer_threads;
    
    // Then use actual_tsi_config when starting writer threads:
    for (size_t i = 0; i < contexts.size(); ++i) {
        writer_threads.push_back(std::make_unique<std::thread>(
            tsi_file_writer_thread_v2,  // Use the new version
            std::ref(contexts[i]),
            std::ref(stop_writing_flags[i]),
            std::ref(writer_stats[i]),
            std::cref(actual_tsi_config)));  // Pass config with anchor
    }
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    if (use_tsi_format) {
        std::cout << "TSI proprietary packet format enabled." << std::endl;
    }
    ...
    // TODO: PPS reset logic still needs help, tsi format PPS reset isn't working fine. The streams are overflowing each time.
    ...
    if (config.pps_reset.enable_pps_reset) {
        if (use_tsi_format) {
            // TSI format handles time sync internally - DO NOT reset here!
            std::cout << "\n[TSI Mode] Time synchronization will be handled during capture setup." << std::endl;
            pps_reset_used = true;  // Flag for downstream code
        } else {
            // Non-TSI mode: perform legacy PPS reset here
    ...
    try {
        if (use_tsi_format) {
            ...
            tsi_config.include_file_header = false;
            tsi_config.csv_max_packets = tsi_csv_packets;
            tsi_config.csv_samples_per_packet = tsi_csv_samples;

            // Call the TSI capture function which handles its own time sync
            ...
            if (format == "sc16") {
                capture_multi_stream_unified<std::complex<short>>(graph,
                    config, file, num_packets, enable_analysis, csv_file,
